...
function checkRawHtml(line, lineNum, errors, filePath) {
  ...
  while ((match = HTML_TAG_REGEX.exec(stripped)) !== null) {
    ...
    if (fullTag.startsWith('<!--') || tagName === 'http' || tagName === 'https') {
      continue;
    }
    errors.push(
...
export function validateMarkdownContent(content, filePath = 'markdown') {
  ...
  if (state.inCode) {
    errors.push(`${filePath}:EOF - Unclosed code block opened at line ${state.codeLine}.`);
  }
...
function runCli() {
  ...
  const targetFiles =
    args.length > 0 ? args.map((f) => resolve(process.cwd(), f)) : findMarkdownFiles(process.cwd());
  ...
  if (totalErrors > 0) {
    ...
    console.log(`✅ Markdown structure & purity verified across ${targetFiles.length} file(s).`);
  }
}
