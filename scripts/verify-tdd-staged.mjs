...
export function checkTddStaged(stagedFiles) {
  ...
  const stagedTestFiles = stagedFiles.filter((file) => TEST_FILE_REGEX.test(file));
...
function runCli() {
  ...
  if (!pass) {
    ...
    console.error('Stage accompanying test files (*.test.ts, tests/**) to proceed with commit.\n');
    process.exit(1);
  }
}
