  ...
  rules: {
    'no-hardcoded-colors': {
      meta: {
        type: 'problem',
        docs: {
          description: 'Enforce design tokens and CSS variables over hardcoded colors.',
        },
        messages: {
          noRawColor:
    ...
    'enforce-motion-tokens': {
      meta: {
        type: 'problem',
        docs: {
          description: 'Enforce Material Design 3 motion tokens for springs and easing.',
        },
        messages: {
          noHardcodedSpring:
        ...
        return {
          Property(node) {
            ...
            if (key === 'ease' || key === 'easing') {
              if (typeof node.value?.value === 'string' && /cubic-bezier/i.test(node.value.value)) {
                context.report({
                  node: node.value,
    ...
    'enforce-typography-tokens': {
      meta: {
        type: 'problem',
        docs: {
          description: 'Enforce typography tokens over raw ad-hoc font families.',
        },
        messages: {
          noRawFontFamily:
      ...
      create(context) {
        return {
          Property(node) {
            ...
            const val = node.value?.value;
            if (typeof val !== 'string') return;
            if (val.includes('var(--font-family-') || val === 'inherit' || val === 'initial')
              return;
