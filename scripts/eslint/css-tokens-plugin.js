...
function isOverrideSelector(selector) {
  ...
  return selector.children.some((child) => {
    if (child.type === 'ClassSelector') {
      const n = child.name;
      return n === 'override' || n.startsWith('override-') || n.endsWith('-override');
    }
...
function checkRuleSelectors(node, context) {
  ...
  for (const selector of node.prelude.children) {
    ...
    for (const child of selector.children || []) {
      ...
      } else if (child.type === 'PseudoElementSelector' && child.name === 'part') {
...
function isNativeComponent(name) {
  return typeof name === 'string' && (name.startsWith('m3e-') || name.startsWith('md-'));
}

function isUniversalSelector(selector) {
  return selector.children?.some((c) => c.type === 'TypeSelector' && c.name === '*');
}
  ...
  rules: {
    'no-unscoped-component-override': {
      meta: {
        type: 'problem',
        docs: {
          description: 'Disallow direct, un-scoped overrides of native MD3 web components.',
        },
        messages: {
          unscopedComponent:
    ...
    'no-scoped-important': {
      ...
      create(context) {
        ...
        return {
          Rule(node) {
            currentRuleHasOverride = node.prelude?.children?.some(isOverrideSelector) ?? false;
          },
          'Rule:exit'() {
    ...
    'no-raw-colors': {
      meta: {
        type: 'problem',
        docs: {
          description: 'Forbid raw hex/rgb/hsl colors in CSS in favor of tokens and CSS variables.',
        },
        messages: {
          noRawColor:
        ...
        return {
          ...
          Declaration(node) {
            ...
            for (const child of node.value?.children || []) {
              if (child.type === 'Identifier' && !ALLOWED_COLOR_IDS.has(child.name.toLowerCase())) {
                context.report({
                  node: child,
                  messageId: 'noRawColor',
    ...
    'no-unperformant-transitions': {
      ...
      create(context) {
        ...
        return {
          Rule(node) {
            currentRuleIsUniversal = node.prelude?.children?.some(isUniversalSelector) ?? false;
          },
          'Rule:exit'() {
    ...
    'no-raw-font-family': {
      meta: {
        type: 'problem',
        docs: {
          description: 'Forbid raw font-family declarations without tokens or CSS variables.',
        },
        messages: {
          noRawFont:
      ...
      create(context) {
        return {
          Declaration(node) {
            ...
            if (
              !text.includes('var(') &&
              !['inherit', 'initial', 'unset', 'system-ui'].includes(text.trim())
            ) {
              context.report({
                node,
                messageId: 'noRawFont',
    ...
    'no-tailwind-directives': {
      meta: {
        type: 'problem',
        docs: {
          description: 'Forbid Tailwind CSS directives (@theme, @apply, @import "tailwindcss").',
        },
        messages: {
          noTailwind:
    ...
    'enforce-theme': {
      meta: {
        ...
        docs: {
          description:
            'Enforce that all CSS colors originate strictly from the Solarized theme system (--theme-*, --colors-*, --md-sys-color-*, --m3e-*), exempting only background canvas shaders.',
        },
        messages: {
          invalidThemeColor:
      ...
      create(context) {
        ...
        return {
          Rule(node) {
            ...
            isBackgroundCanvas =
              /#season-canvas|\.season-hero-canvas|\.season-hero-wrapper|\.season-background/.test(
                selectorText,
              );
          },
          'Rule:exit'() {
              ...
              if (child.type === 'Hash') {
                ...
                if (COLOR_FUNCS.has(funcName)) {
                  ...
                  if (varMatch) {
                    ...
                    if (!isThemeToken && (COLOR_PROPS.has(prop) || prop.endsWith('-color'))) {
                      context.report({
                        node: child,
                        messageId: 'invalidThemeColor',
                        data: { value: varName },
                    ...
                    if (fallback && (fallback.startsWith('#') || /^(rgb|hsl)/i.test(fallback))) {
                      context.report({
                        node: child,
